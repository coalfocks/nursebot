// Script to add weight_kg column to patients and populate with generated weights
// Run with: node scripts/add-patient-weights.js
...
function generateWeight(age, sex) {
  ...
  if (age >= 18) {
    ...
    // Children: CDC growth chart percentiles (50th-90th)
    const weightChart = {
...
async function main() {
  console.log('Adding weight_kg column...');

  // Add column if it doesn't exist (using raw SQL via RPC)
  const { error: alterError } = await supabase.rpc('exec_sql', {
    query: 'ALTER TABLE public.patients ADD COLUMN IF NOT EXISTS weight_kg numeric;'
  });

  if (alterError) {
    console.log('Column may already exist or RPC not available, continuing...');
  } else {
  ...
  // Update each patient with a generated weight
  let updated = 0;
  for (const patient of patients) {
    ...
    if (updateError) {
      ...
      updated++;
      console.log(`Updated patient ${patient.id}: age=${age}, sex=${patient.gender}, weight=${weightKg}kg`);
    }
  }

  console.log(`\n✅ Updated ${updated}/${patients.length} patients with weight_kg`);
}
